...
class Vue():
    ...
    #### Creation des canvas ####
        ...
        self.main_map = pygame.Surface((self.map_width, self.map_height))
        self.fond = pygame.image.load("img/test.jpg").convert()
        self.main_map = self.fond.convert()
...
#### Refresh du canvas complet ####
    def refresh(self):
        ...
        self.main_map.blit(self.solo, (posSolo[0], posSolo[1]))
        ...
        for key in self.listImgEnnemis:
            if key == "droneA":
                for item1 in self.listImgEnnemis[key]:
                    ...
                    self.main_map.blit(item1[1], (posDroneA[0], posDroneA[1]))
            elif key == "droneS":
                for item1 in self.listImgEnnemis[key]:
                    self.projectile = item1[1].get_rect()
                    ...
                    self.main_map.blit(item1[1], (posDroneS[0], posDroneS[1]))
            elif key == "camera":
                for item1 in self.listImgEnnemis[key]:
                    self.projectile = item1[1].get_rect()
                    ...
                    self.main_map.blit(item1[1], (posCamera[0], posCamera[1]))

        for item in self.listImgProjectileEnnemi:
            self.projectile = item[1].get_rect()
            posProjectile = item[0].position
            self.main_map.blit(item[1], (posProjectile[0], posProjectile[1]))
        ...
        self.fenetre.blit(self.main_map, (self.map_x, self.map_y, self.window_width, self.window_height))
        self.main_map = self.fond.convert()
        #self.main_map.blit(self.fond, (self.window_width, self.window_height, self.map_x, self.map_y))
        pygame.display.flip()
        #pygame.display.update(self.fenetre)
